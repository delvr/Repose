...
import cpw.mods.fml.common.registry.GameData._
...
object ReposeConfig extends ConfigCategory(None, ReposeMod.name) {
    ...
    val avalanches = new BooleanSetting(this, "Avalanches",
        s"If true and block spread is enabled, non-soil granular blocks (such as sand or gravel) will also trigger avalanches when disturbed. " +
...
class BlockSetSetting(category: ConfigCategory, name: String, help: String, defaultValue: () => Set[Block])
    ...
    protected def writeElement(b: Block) = getBlockRegistry.getNameForObject(b)
}
